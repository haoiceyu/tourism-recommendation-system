...
async function loadCategories() {
  const { data } = await http.get<Category[]>('/categories', { params: { scope: 'HOTEL' } })
...
async function load() {
  loading.value = true
  try {
    const { data } = await http.get<PageResp<Hotel>>('/hotels', {
      params: {
  ...
  <div class="page-wrap">
    <el-card shadow="never">
      ...
      <el-row :gutter="16" v-loading="loading">
        <el-col v-for="h in list" :key="h.id" :xs="24" :sm="12" :md="8">
          <el-card class="item" shadow="hover" @click="open(h.id)">
            ...
            <div v-if="h.avgRating != null" class="meta">评分 {{ h.avgRating.toFixed(1) }}</div>
          </el-card>
        </el-col>
      </el-row>
